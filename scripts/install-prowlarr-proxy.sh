...
#
# curl -fsSL https://raw.githubusercontent.com/killamfkr/warpbox/cursor/casaos-install-script-1b99/scripts/install-prowlarr-proxy.sh -o /tmp/p.sh && sed -i 's/\r$//' /tmp/p.sh && chmod +x /tmp/p.sh && sudo bash /tmp/p.sh
...
mkdir -p "${PROXY_DIR}"
curl -fsSL "https://raw.githubusercontent.com/killamfkr/warpbox/cursor/casaos-install-script-1b99/scripts/prowlarr-torrent-proxy.py" \
